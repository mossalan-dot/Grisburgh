...
export const api = {
  ...
  markt:          ()           => request('/markt'),
  zetMarktGebieden: (gebieden) => request('/meta/markt', { method: 'PUT', body: JSON.stringify({ gebieden }) }),
  ...
  spotifyOntkoppel: ()            => request('/spotify/ontkoppel', { method: 'POST' }),
  spotifyApparaten: ()            => request('/spotify/apparaten'),
  spotifyZoek:      (q)           => request(`/spotify/zoek?q=${encodeURIComponent(q)}`),
  spotifySpeel:     (data)        => request('/spotify/speel', { method: 'POST', body: JSON.stringify(data) }),
  spotifyPauze:     ()            => request('/spotify/pauze', { method: 'POST' }),
  spotifyToken:     ()            => request('/spotify/token'),
  spotifyTafelApparaat: (deviceId) => request('/spotify/tafel-apparaat', { method: 'POST', body: JSON.stringify({ deviceId }) }),
