...
export function initSocket() {
  ...
  socket.on('archief:updated', () => {
    ...
    if (section === 'documenten') {
      import('./render-archief.js?v=75').then(m => m.renderDocumenten());
    } else if (section === 'logboek') {
      import('./render-archief.js?v=75').then(m => m.renderLogboek());
    }
  ...
  socket.on('dungeon:revealed', () => {
    ...
    import('./render-dungeon.js?v=33').then(m => {
  ...
  socket.on('meta:updated', () => {
    import('./api.js?v=262').then(({ api }) => api.meta().then(m => {
