...
function _ververOpenKaartje(id) {
  ...
  socket.on('archief:updated', () => {
    if (window.app.state.activeSection === 'logboek') {
      import('./render-archief.js?v=104').then(m => m.renderLogboek());
    }
  });
  ...
  // Voorleestekst uit een akte. Alleen het tafelscherm toont hem: de spelers
  // horen hem, ze hoeven hem niet op hun telefoon mee te lezen.
  socket.on('display:tekst', ({ tekst, kop } = {}) => {
    if (window._isDisplayMode) window._displayTekst?.(tekst, kop);
  });
  ...
  socket.on('map:pinRevealed', () => {
    ...
    if (window.app.state.activeSection === 'kaart') {
      import('./render-kaart.js?v=31').then(m => m.renderKaart());
    }
  });
  ...
  socket.on('meta:updated', () => {
    import('./api.js?v=286').then(({ api }) => api.meta().then(m => {
