...
import { initCampagne, renderPersonages, renderLocaties, renderOrganisaties, renderVoorwerpen, renderDocumenten, openEditor, WEAPON_PROPERTIES, PARAMETERIZABLE_PROPS } from "./render-campagne.js?v=313";
...
import { initDmPanel } from "./dm-panel.js?v=269";
...
const _DIENST_STANDAARD = {
  ...
  tempel: 'Tempel', magizoo: 'Magizoöloog', facties: 'Facties',
};
...
function applyRole() {
  ...
  const DIENSTEN_SECTIONS = ['markt', 'herberg', 'tweespalt', 'gock', 'ursula', 'tempel', 'facties', 'magizoo'];
...
async function renderMijnKarakter(opts = {}) {
  ...
  let lootData = null;
  try {
    [hpData, entity, combat, ownershipData, allVoorwerpen, soundsData, simpleItems, currency, partyCurrency, spellSlots, playerProfile, partyMembers, companions, trackers, pinnedSpells, pinnedTraits, { inspired }, berichtenLijst, factiesData, progData, lootData] = await Promise.all([
  ...
  el.innerHTML = `
    <div class="player-dashboard"${_themeAttr}>
      ...
      <div id="pst-facties" class="player-subtab-panel${_playerSubTab !== 'facties' ? ' hidden' : ''}">
        ...
        ${!(factiesData || []).some(f => (f.rang?.index ?? 0) > 0)
...
const HELP_CONFIG = {
  ...
  hulp_kijk_personages_info: () => ({ titel: 'Dit personage', stappen: [
    ...
    { titel: 'Waar hoort hij bij', tekst: 'Onder **Waar hoort dit personage bij?** staat waaraan hij verbonden is: eigenaar van een herberg, lid van een gilde, verkoper in een winkel. Klik erop om daarheen te gaan.', afbeelding: null },
  ] }),
  ...
  hulp_kijk_organisaties_info: () => ({ titel: 'Deze organisatie', stappen: [
    { titel: 'Wat je ziet', tekst: 'Het type en het motto staan onder de naam; de beschrijving vertelt waar ze voor staan. Onder **Wie hoort hier bij?** staan de leden die je kent, met hun rol of rang; onder **Waar hoort deze organisatie bij?** staat waar de organisatie zelf onder valt — en of ze een **factie** zijn waar je aanzien bij kunt opbouwen.', afbeelding: null },
  ...
  hulp_bewerk_personages_info: () => ({ titel: 'Een personage', stappen: [
    ...
    { titel: 'Waar hoort dit personage bij?', tekst: 'Waar dit personage bij hoort: eigenaar van een herberg, lid van een gilde. Kies een locatie of organisatie plus een rol met **Verbinding toevoegen**. Diezelfde regel komt te staan bij *Wie hoort hier bij?* van dát kaartje — een verbinding staat maar op één plek, dus je kunt hem van beide kanten leggen en van beide kanten weghalen. Onderaan staat **Verkoopt bij**: dat is dezelfde soort verbinding, maar dan naar de winkel waar zijn waren liggen.', afbeelding: null },
...
// Bestaat er uitleg voor deze sleutel? De knop hoort niet te verschijnen als er
// niets achter zit.
window._heeftHelp = (key) => !!_resolveHelp(key);
