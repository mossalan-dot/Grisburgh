    ...
    if (isEntity) {
      ...
      return `
        <g class="rel-node${sel ? ' rel-node--selected' : ''}" data-id="${esc(node.id)}"
          ...
          ${imgSvg}
          ${crossSvg}
          ${hasNotes ? `<use href="/img/icons.svg?v=17#icon-pencil" x="${NODE_W - 13}" y="3"
      ...
      return `
        <g class="rel-node${sel ? ' rel-node--selected' : ''}" data-id="${esc(node.id)}"
          ...
          ${hasNotes ? `<use href="/img/icons.svg?v=17#icon-pencil" x="${NODE_W - 13}" y="${ofY + 5}"
            width="9" height="9" style="color:#7a6040;pointer-events:none"/>` : ''}
          ${lines.length === 1
...
function _silhouetSvg(type, cx, cy, maat) {
  ...
  return `<use href="/img/icons.svg?v=17#icon-${naam}" x="${cx - maat / 2}" y="${cy - maat / 2}"
