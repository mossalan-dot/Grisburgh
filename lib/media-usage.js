...
function _entitiesLabels(data, id) {
  const out = [];
  const TYPE_LABEL = { personages: 'Personage', locaties: 'Locatie', organisaties: 'Organisatie', voorwerpen: 'Voorwerp', documenten: 'Document' };
...
function _archiefLabels(data, id) {
  const out = [];
  // Documenten zijn kaartjes geworden; die worden in entities.json gevonden.
...
function scanUsage(fileIds = null) {
  ...
  // Speciale conventie: de portretvideo van een speler-personage staat op
  // `{entityId}_video` en wordt NERGENS als dataveld opgeslagen (auth.js leidt
  // hem af uit het bestaan van het bestand). Zonder deze check zou de generieke
  // scan zo'n video als 'wees' zien → de DM kan het startscherm-filmpje per
  // ongeluk verwijderen. Markeer hem dus expliciet als in gebruik.
  const idsToCheck = idSet ? [...idSet] : Object.keys(usage);
  const videoIds = idsToCheck.filter(id => id.endsWith('_video'));
  if (videoIds.length) {
    let ents; try { ents = storage.readJSON('entities.json'); } catch { ents = {}; }
    const personages = ents.personages || [];
    for (const vid of videoIds) {
      const baseId = vid.slice(0, -'_video'.length);
      const e = personages.find(p => p.id === baseId);
      if (e) { (usage[vid] = usage[vid] || []).push(`Personage: ${e.name || e.id} · startscherm-video`); }
    }
  }
