...
function _mapLabels(data, id) {
  const out = [];
  for (const m of (data.maps || [])) if (_deepHasValue(m, id)) out.push(`Kaart: ${m.name || m.id}`);
  for (const p of (data.pins || [])) if (_deepHasValue(p, id)) out.push(`Kaartpin: ${p.name || p.id}`);
  return out;
}
