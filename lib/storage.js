...
// Bestaat deze campagne? Gebruikt om een campagne-id uit een URL of body te
// controleren voordat er iets mee gedaan wordt — zo kan niemand met een
// verzonnen naam buiten de campagnemappen komen.
function campagneBestaat(id) {
  if (!id || !/^[a-z0-9_-]+$/i.test(id)) return false;
  try { return fs.statSync(path.join(CAMPAIGNS_DIR, id)).isDirectory(); } catch { return false; }
}
...
function createCampaign(id, meta = {}) {
  ...
  const defaults = {
    ...
    'archief.json':     { logEntries: [], sessieLog: [] },
    'dm-state.json':    { activeGroup: 'groep1', groups: { groep1: { name: 'Groep 1', visibility: {}, secretReveals: {}, deceased: {}, itemOwners: {}, itemRequests: [], tradeAllowed: true } }, dmNotes: {}, playerCurrency: {}, playerSpellSlots: {}, playerItems: {}, playerProfiles: {}, playerTrackers: {}, playerSpells: {}, playerInspiration: {}, trash: [] },
    ...
    'tables.json':      { tables: [] },
    'meta.json':        { appTitle: id, appSubtitle: '', theme: 'default', currency: { ...MUNT_STANDAARD }, spellSource: 'dnd5e', hoofdstukken: {}, ...meta },
  };
...
// De standaardmunt is D&D-generiek; de eigen munt van een campagne staat in
// haar meta.json. De verhouding blijft 1:10:100, dus de sleutels fl/kn/cl zijn
// gewoon de gouden, zilveren en koperen plek — alleen de naam erboven verschilt.
const MUNT_STANDAARD = { fl: 'Gold', kn: 'Silver', cl: 'Copper' };

const DEFAULTS = {
  'entities.json':    { personages: [], locaties: [], organisaties: [], voorwerpen: [], documenten: [] },
  'archief.json':     { logEntries: [], sessieLog: [] },
  'dm-state.json':    { visibility: {}, secretReveals: {}, dmNotes: {}, deceased: {} },
  'map.json':         { pins: [] },
  'tables.json':      { tables: [] },
  'combat.json':      { active: false, round: 1, currentTurn: 0, combatants: [] },
  'player-notes.json':{},
  'meta.json':        { appTitle: 'Grisburgh', appSubtitle: '', theme: 'default', currency: { ...MUNT_STANDAARD }, spellSource: 'dnd5e', hoofdstukken: {} },
...
// In welke campagne draait dit verzoek? Nodig om te controleren of een sessie
// hier iets te zoeken heeft: een DM van campagne A is op /B gewoon een bezoeker.
function huidigeCampagne() { return path.basename(_curDataDir()); }
...
// Serveer-map: extensie op schijf → Content-Type. Bevat óók de "rauwe" extensies waarmee
// oudere audio-uploads per ongeluk zijn opgeslagen (x-m4a, x-wav) door de split-fallback in
// saveFile — zo krijgen bestaande bestanden alsnog een geldige audio-MIME i.p.v. octet-stream,
// wat de browser nodig heeft om ze af te spelen.
const EXT_TO_MIME = {
  png: 'image/png', jpg: 'image/jpeg', jpeg: 'image/jpeg', gif: 'image/gif',
  webp: 'image/webp', avif: 'image/avif', svg: 'image/svg+xml', pdf: 'application/pdf',
  mp3: 'audio/mpeg', ogg: 'audio/ogg', oga: 'audio/ogg',
  wav: 'audio/wav', 'x-wav': 'audio/wav', wave: 'audio/wav',
  m4a: 'audio/mp4', 'x-m4a': 'audio/mp4', aac: 'audio/aac',
  flac: 'audio/flac', 'x-flac': 'audio/flac',
...
// Bestaat er een bestand voor dit id? Alleen de vraag, niet het bestand — de
// kaartjes willen vóór het renderen weten of er een afbeelding is, anders
// reserveren ze ruimte voor een plaatje dat er nooit komt. De mappenlijst wordt
// per map gecachet op mtime, zodat honderd kaartjes niet honderd readdirs zijn.
const _bestandenCache = new Map();   // dir → { mtimeMs, exts:Map<id, ext> }
function _bestandenIndex() {
  const dir = _curFilesDir();
  const mtimeMs = fs.statSync(dir).mtimeMs;
  let c = _bestandenCache.get(dir);
  if (!c || c.mtimeMs !== mtimeMs) {
    const exts = new Map();
    for (const f of fs.readdirSync(dir)) {
      const punt = f.lastIndexOf('.');
      exts.set(punt > 0 ? f.slice(0, punt) : f, punt > 0 ? f.slice(punt + 1).toLowerCase() : '');
    }
    c = { mtimeMs, exts };
    _bestandenCache.set(dir, c);
  }
  return c.exts;
}
function bestandBestaat(id) {
  if (!id) return false;
  try { return _bestandenIndex().has(String(id)); } catch { return false; }
}

// Niet élk bestand onder een kaartje-id is een afbeelding: bij een document
// hangt er vaak een pdf-scan of een geluidsfragment onder datzelfde id. Wie wil
// weten of er een *plaatje* is, moet dat apart vragen — anders reserveert een
// kaartje ruimte voor beeld dat nooit komt, en probeert een onthulling een pdf
// als <img> te laden (gebroken icoon).
const _BEELD_EXT = new Set(['png', 'jpg', 'jpeg', 'webp', 'avif', 'gif', 'svg']);
function bestandIsBeeld(id) {
  if (!id) return false;
  try { return _BEELD_EXT.has(_bestandenIndex().get(String(id)) || ''); } catch { return false; }
}
...
// Verwijder de gecachte WebP-thumbnail van een bestand (data/campaigns/<id>/thumbs/).
// Ook de vervaagde variant (`.waas.webp`) die een vaag kaartje of document
// krijgt — anders blijft de waas van een vervangen afbeelding hangen.
function deleteThumb(id) {
  // .w1200 is de brede variant (aktebanners); die hoort net zo goed weg te
  // vallen als het beeld eronder vervangen wordt.
  for (const naam of [`${id}.webp`, `${id}.w1200.webp`, `${id}.waas.webp`]) {
    try { fs.unlinkSync(path.join(_curDataDir(), 'thumbs', naam)); } catch {}
  }
}
...
module.exports = {
  huidigeCampagne,
  MUNT_STANDAARD,
  init, readJSON, writeJSON, saveFile, getFile, bestandBestaat, bestandIsBeeld, deleteFile, deleteThumb, campagneBestaat,
