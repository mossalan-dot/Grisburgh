...
const _bestandenCache = new Map();   // dir → { mtimeMs, ids:Set }
function bestandBestaat(id) {
  ...
  try {
    ...
    if (!c || c.mtimeMs !== mtimeMs) {
      c = { mtimeMs, ids: new Set(fs.readdirSync(dir).map(f => f.replace(/\.[^.]+$/, ''))) };
      _bestandenCache.set(dir, c);
    }
    return c.ids.has(String(id));
  } catch { return false; }
}
...
module.exports = {
  huidigeCampagne,
  MUNT_STANDAARD,
  init, readJSON, writeJSON, saveFile, getFile, bestandBestaat, deleteFile, deleteThumb, campagneBestaat,
