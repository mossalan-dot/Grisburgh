...
// `feats-2024` is een kále namenlijst (general/origin/fighting style + Epic
// Boons) — structuur, geen tekst, dus die mag elke campagne hebben.
const BRONNEN = ['spells-2024', 'hp-spells', 'extra-spells', 'feature-descriptions', 'backgrounds-2024', 'volken-klassen', 'srd-monsters', 'srd-items', 'srd-magic-items', 'feats-2024'];
...
function bronVoor(naam, { volledig, eigen = {} } = {}) {
  ...
  // De standaard-statblokken zijn **volledig** SRD 5.2 en dus CC BY 4.0: die
  // gaan bij élke campagne compleet de deur uit. Geen kale variant nodig — dat
  // onderscheid bestaat alleen voor bestanden waar PHB-tekst in zit.
  if (naam === 'srd-monsters') return data;
  // Zelfde verhaal voor de uitrusting en de magische voorwerpen: SRD 5.2, dus
  // CC BY 4.0 en compleet mee naar elke campagne. Ze staan er als **bron om uit
  // te putten**, niet als kaartjes: een magisch voorwerp hoort pas in het
  // archief als de DM het in zijn wereld heeft neergelegd.
  if (naam === 'srd-items' || naam === 'srd-magic-items') return data;
