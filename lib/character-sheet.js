...
// Vakjes om met pen in te vullen: death saves, exhaustion, spell slots. Houdt
// de app er zelf een stand van bij (exhaustion), dan komen de eerste `aan`
// vakjes gevuld op het blad — het blad is de stand van dát moment.
const vakjes = (n, cls = '', aan = 0) => Array.from({ length: n },
  (_, i) => `<span class="vak ${cls}${i < aan ? ' vak--aan' : ''}"></span>`).join('');
...
// De beurs kent drie munten: fl, kn en cl (1:10:100). Electrum en platinum
// hebben géén eigen plek — die worden bij het invoeren omgerekend (zie
// `parsePrijs` in routes/api.js). Dit blok liep over *alle* sleutels in
// `meta.currency`, en omdat een DM daar ook een naam voor ep en pp kan invullen
// stond er op elk uitgedeeld blad "0 Lion d'or · 0 Gouden Adelaer" — twee
// vakjes die per definitie altijd nul zijn.
const BEURS_MUNTEN = ['fl', 'kn', 'cl'];

function beursBlok(beurs, muntNamen) {
  const rij = (waarden, label) => `
    <div class="beurs__rij">
      <span class="beurs__label">${label}</span>
      ${BEURS_MUNTEN.map(sleutel =>
        `<span class="munt"><b>${getal((waarden || {})[sleutel])}</b> ${esc((muntNamen || {})[sleutel] || sleutel)}</span>`).join('')}
    </div>`;
...
function blad(p) {
  ...
  const hp      = p.hp || {};
  // Een multiclass hoort er voluit te staan. Deze regel toonde alleen de
  // hoofdklasse met het tótale level ("Cleric (Light Domain) · Level 8"),
  // waardoor de Wizard 3 van Wendel Tweepad nergens op zijn blad stond — terwijl
  // de Hit Dice eronder wél netjes 5d8 · 3d6 lieten zien. Het dashboard in de
  // app doet dit al goed (`_klasseStr`); het blad liep achter.
  const _isMulti = (profiel.multiclass === true || profiel.multiclass === 'true') && profiel.multiKlasse;
  const _helft = (kl, lvl, sub) =>
    [kl || '?', lvl && String(lvl), sub && `(${sub})`].filter(Boolean).join(' ');
  const _klasseDeel = _isMulti
    ? [_helft(profiel.klasse, profiel.klasseLevel, profiel.subclass),
       _helft(profiel.multiKlasse, profiel.multiKlasseLevel, profiel.multiSubclass)].join(' / ')
    : _helft(profiel.klasse, null, profiel.subclass);
  const klasseRegel = [
    _klasseDeel,
    // Bij een multiclass staat het level al per klasse hierboven; het totaal
    // erbij zetten leest dan als een derde getal dat nergens bij hoort.
    !_isMulti && profiel.level && `Level ${profiel.level}`,
    profiel.origin,
    profiel.background,
    // Speelt de campagne op XP, dan hoort het getal op het blad: anders moet je
    // het bij elke sessie in de app opzoeken. Op mijlpaal staat er niets — daar
    // ís geen getal.
    p.xp && (p.xp.volgende != null
      ? `${p.xp.nu.toLocaleString('nl-NL')} / ${p.xp.volgende.toLocaleString('nl-NL')} XP`
      : `${p.xp.nu.toLocaleString('nl-NL')} XP`),
  ].filter(Boolean).join(' · ');
    ...
    <div class="kolommen">
      ...
      <section class="kol">
        ...
        <div class="hd">
          ...
          <div><span class="lab">Exhaustion</span>${vakjes(6, '', p.exhaustion || 0)}${
            p.exhaustion ? `<span class="klein"> −${p.exhaustion} op elke d20 test</span>` : ''}</div>
        </div>
...
/* Een vakje dat de app al weet: gevuld, niet meer open om in te tikken. */
.vak--aan { background: #8a6d3f; }
