...
## 0b. Gekozen richting (juni 2026): NL besloten community

Vastgelegd met de DM: **een besloten groep bekende DM's deelt één instance**, elk met eigen
campagnes, **plus content uitwisselen** tussen DM's. Taal blijft **Nederlands**.

Dit zit **tussen A en B in** — "lichte multi-tenant". Gevolgen voor dit plan:
- 🟢 **i18n vervalt** (§3d) — de grootste *tedious* klus (~1.500 strings) valt weg. Blijft NL.
- 🟡 Wél **per-DM eigenaarschap + toegangscontrole**, géén volledig accountsysteem / AVG-zwaar
  SaaS. Nieuwe pijler **§8**.
- 🟢 **Content delen** wordt een eigen pijler: export/import + gedeelde bibliotheek +
  template-campagnes (Grisburgh als showcase). Nieuwe pijler **§9**.
- 🟢 Lore loskoppelen (§1) + valuta/thema **per campagne** (§3a/b) blijven nodig — elke DM wil een
  eigen wereld en eigen look op dezelfde instance.
- ⚪ Volledig open platform (§5/ambitie B) is **bewust niet gekozen**.
...
### 3d. Taal / i18n — ~~**L**~~ → **VERVALT** (NL besloten community, zie §0b)

> Niet meer nodig nu de doelgroep NL is. Onderstaande bewaard als referentie mocht Engels later
> alsnog gewenst zijn.
...
## 8. Lichte multi-DM tenancy  *(gekozen richting — §0b)*  — **M**

Huidig: één gedeeld `DM_PASSWORD` (`config.js`, `routes/auth.js:15`); campagnes al map-geïsoleerd
(`data/campaigns/<id>/`); een per-sessie `campaignId`-override bestaat (nu vooral voor sandbox).

Nodig voor "meerdere DM's, eigen campagnes op 1 instance":
- **DM-register** op instance-niveau (bv. `data/dms.json`: `id`, naam, wachtwoord-hash). Sessie krijgt
  een `dmId` naast `role:'dm'`.
- **Campagne-eigenaarschap:** `meta.ownerDmId` (gezet bij `createCampaign`, `storage.js:54`) of een
  registry `campaignId → dmId`.
- **Toegangscontrole server-side afdwingen:** een DM kan alleen z'n eigen (of gedeelde) campagnes
  openen/bewerken. In de campagne-middleware controleren — **niet** alleen in de UI verbergen.
- **Campagnekeuze na login:** een "kies of maak een campagne"-scherm dat alleen jouw campagnes toont
  (bouwt voort op `POST /campaigns` + de bestaande campagne-switcher in `dm-panel.js`).
- **Spelers ongewijzigd:** per-character + groepswachtwoord, al campagne-scoped (`routes/auth.js:103`).

Bewust **niet** (besloten/vertrouwde groep): e-mailverificatie, wachtwoordreset-flows, billing,
rate-limiting-zwaar. Wel verplicht: `SESSION_SECRET` netjes zetten en eigenaarschap **echt**
afdwingen (ook vertrouwde DM's mogen elkaars data niet per ongeluk overschrijven).

> **Aandachtspunt:** de actieve-campagnekeuze is nu deels **globaal** (`active-campaign.json` in
> `storage.js`). Voor meerdere gelijktijdige DM's moet de keuze **per sessie** i.p.v. globaal —
> uitzoeken hoe `setCampaign`/`loadActiveCampaign` zich verhoudt tot de session-override (`server.js`).

---

## 9. Content delen tussen DM's  *(gekozen richting — §0b)*  — **M–L**

- **Campagne-export/import:** bundel de JSON-bestanden **+ `files/`-uploads** in één archief; import
  maakt een nieuwe campagne. Dient meteen als back-up én als template.
- **Bouwsteen-delen:** monsters, encounters, tabellen, statblocks, kaarten, voorwerpen los
  export-/importeerbaar — of een **gedeelde bibliotheek** op de instance waar DM's uit putten.
- **Template-campagnes:** "nieuwe campagne op basis van &lt;template&gt;"; **Grisburgh als
  showcase-template** bewaren (sluit aan op §2).
- **Asset-ID's hermappen bij import** om botsingen tussen campagnes te voorkomen (uploads moeten
  meereizen met de bundel).
- **Rechten + veiligheid:** wie mag naar de gedeelde bibliotheek publiceren; gedeelde content is
  **kopiëren bij import, niet live-bewerken** (veiligst — geen kruislingse mutaties).
- Bouwt voort op de bestaande per-campagne `files/`-mappen en de volledig serialiseerbare
  JSON-structuur — technisch een goede uitgangspositie.
...
| **T1** | Lichte multi-DM tenancy: eigenaarschap + toegangscontrole + campagnekeuze (§8) | M | Meerdere DM's, eigen campagnes op 1 instance |
| **S1** | Content delen: campagne-export/import + template-campagnes (§9) | M | Campagnes/bouwstenen uitwisselen |
| **S2** | Gedeelde community-bibliotheek op de instance (§9) | M–L | Pool van deelbare content |
| ~~A4~~ | ~~i18n + EN~~ — **vervalt** (NL-only, §0b) | — | — |
| **A5** | Diensten volledig data-driven (§1a volledig) | L | Diensten zonder code toevoegen |
| ~~B~~  | ~~Open multi-tenant + hosting/AVG~~ — **niet gekozen** | XL | (open platform) |

> **Gekozen mijlpaal (NL besloten community):** **A1 + A2 + T1 + S1** → meerdere bekende DM's delen
> één instance, elk met eigen campagnes (eigen naam/valuta/thema/diensten, lore-vrij), en kunnen
> campagnes/templates uitwisselen. **S2** (gedeelde bibliotheek) en **A5** zijn fijne uitbreidingen
> daarna; **A4** (Engels) en **B** (open platform) zijn bewust niet gekozen.

---

## Belangrijkste beslissingen (voor de DM)

1. ~~Ambitie~~ — **beslist:** besloten NL-community, 1 gedeelde instance, lichte tenancy (§0b, §8).
2. ~~Taal~~ — **beslist:** NL-only (i18n vervalt).
3. **Diensten:** pragmatisch hernoembaar (snel) of volledig data-driven (vrij maar L)? *(open)*
4. **Content delen:** alleen export/import-bestanden, of ook een in-app gedeelde bibliotheek (§9)? *(open)*
5. **Eigenaarschap:** mogen DM's elkaars campagnes ook **mede-beheren** (co-DM), of strikt eigen + kopiëren? *(open)*
6. **Valuta/ratio:** alleen namen configureerbaar, of ook denominaties/verhoudingen? *(open)*
7. **SRD:** als templates breder gedeeld worden, content juridisch schonen (§7)? *(open)*
